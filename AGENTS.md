...
- Do not set global model defaults; let each user pick preferred models and persist their settings. Use Oxen’s live catalog and keep controls consistent across models.
- Brand the product as DS Studio with the DigiSavvy favicon from digisavvy.com, not “Oxen Studio”.
- Credits pill shows a live dollar balance and links to https://www.oxen.ai/digisavvy/settings/billing; the generate button shows estimated cost and keeps keyboard-hint icons.
...
- Modes (text-to-image, image-to-image, text-to-video, reference-to-video, video-to-video) go through the Oxen async queue only; 1–4 outputs share a `batch_id`, and succeeded results are archived to R2 (`result_key`) then listed with fresh signed URLs.
...
- Catalog should include Seedream 5.0, GPT Image 2.5, Seedance 2.5, Kling 3.0, and Wan 3.0; `GET /api/billing/credits` returns remaining USD (or null) plus the DigiSavvy Oxen billing URL.
